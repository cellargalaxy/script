...
def extract_audio_track(video_path, index, audio_path):
    ...
    logger.info("提取视频音轨,cmd: %s", util.json_dumps(cmd))
    return_code = util.popen_cmd(cmd)
    if return_code != 0:
        logger.error("提取视频音轨，异常")
        raise ValueError("提取视频音轨，异常")
...
def extract_simple_audio(input_path, output_path):
    ...
    logger.info("提取采样音频,cmd: %s", util.json_dumps(cmd))
    return_code = util.popen_cmd(cmd)
    if return_code != 0:
        logger.error("提取采样音频，异常")
        raise ValueError("提取采样音频，异常")
