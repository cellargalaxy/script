"""
1. 我打算使用以下指标对ai翻唱的wav文件进行质量评价。
2. 我只有使用ai翻唱出来的多个wav文件，我能提供这些文件的路径。
3. 判断以下指标，只有wav文件路径，这些文件之间是否能对比出优劣，如果对比不出优劣就不需要再继续了
4. 如果能对比出优劣，写一个python函数，入参是wav文件路径的字符串数组
5. 该python函数实现以下指标的计算，并且将计算结果画为图表进行可视化对比
6. 图表的类型，需要根据指标的特点进行选择，目的是能更加直观的看出各个wav文件的优劣
7. 图表的数轴标度，为了避免不同文件之间的指标差异过小，在图中看不出区别，需要更加明显的处理
8. 文件大约有几十到一百个，需要合理排版，以能清晰看出每个文件的数据走向与图标
9. 文件路径数组已经排好序，按模型的轮数是递增的
10. 在图表中增加该指标的中文文字描述，阈值等辅助信息，使用文件名称标示出各个文件之间的差异
11. 文字描述不能遮住图表本身,将文字描述的背景颜色设置为透明，图表使用常规字体而不是等宽字体
12. 尽量将代码都收敛到函数内部，方便调用，按文件进行并发处理，提升处理速度
13. 最后提供一个完整可用的python函数，以及其需要安装的依赖

+ 音高与基频稳定性（Pitch / F0）
    + 含义：F0曲线是否平滑连续，无频繁断裂/跳变；使用PYIN或CREPE算法提取，评估模型/声码器稳定性（如锯齿状或死点表示AI
      artifact）。
    + 平滑、少自然抖动：正常歌声
    + 锯齿/断层明显：AI抖动或不稳
    + 在长音处应有自然颤音（Vibrato），转音处平滑过渡。
"""

# pip install librosa numpy matplotlib scipy


import os
import numpy as np
import librosa
import matplotlib.pyplot as plt
import matplotlib.ticker as ticker
from concurrent.futures import ProcessPoolExecutor
from matplotlib.font_manager import FontProperties
import warnings

# 忽略librosa在某些极短音频下的警告
warnings.filterwarnings('ignore')


def _analyze_single_file(args):
    """
    单个文件处理函数（用于多进程调用）。
    计算音高不稳定性指标。
    """
    file_path, index = args
    try:
        # 1. 加载音频
        # 使用 sr=16000 提升加载速度，对提取基频影响较小
        y, sr = librosa.load(file_path, sr=16000)

        # 2. 提取基频 (F0) 使用 PYIN
        # fmin/fmax 根据人声范围设定，frame_length设定稍大以获得较稳的结果
        f0, voiced_flag, voiced_probs = librosa.pyin(
            y,
            fmin=librosa.note_to_hz('C2'),
            fmax=librosa.note_to_hz('C7'),
            sr=sr,
            frame_length=2048
        )

        # 如果整首歌都没提取到音高（静音或严重错误），返回None
        if np.all(np.isnan(f0)):
            return index, os.path.basename(file_path), None

        # 3. 计算指标：音高轨迹的平滑度/不稳定性
        # 仅考虑有声片段 (Voiced segments)
        voiced_f0 = f0[~np.isnan(f0)]

        if len(voiced_f0) < 10:
            return index, os.path.basename(file_path), None

        # 将 Hz 转换为 Cents (相对于 C2)，这样差异更符合听感
        # formula: 1200 * log2(f / f_ref)
        f0_cents = 1200 * np.log2(voiced_f0 / librosa.note_to_hz('C2'))

        # 计算一阶差分（相邻帧的音高变化量）
        # 这一步反映了音高的"跳变"程度
        delta_f0 = np.diff(f0_cents)

        # 取绝对值
        abs_delta = np.abs(delta_f0)

        # 计算平均跳变值 (Mean Absolute Variation)
        # 值越小，代表曲线越平滑；值越大，代表锯齿、断层越多
        instability_score = np.mean(abs_delta)

        return index, os.path.basename(file_path), instability_score

    except Exception as e:
        print(f"Error processing {file_path}: {e}")
        return index, os.path.basename(file_path), None


def evaluate_pitch_stability(file_paths, output_image_path="pitch_stability_analysis.png"):
    """
    对一系列WAV文件进行音高稳定性评价并画图。

    参数:
        file_paths (list): 已经按模型轮数排序好的wav文件路径字符串数组。
        output_image_path (str): 输出图片的路径。
    """
    if not file_paths:
        print("文件列表为空。")
        return

    print(f"开始分析 {len(file_paths)} 个音频文件，正在并行计算 F0 指标...")

    # 准备多进程参数，带上索引以保持排序
    tasks = [(fp, i) for i, fp in enumerate(file_paths)]

    results = []
    # 根据CPU核心数进行并行处理
    with ProcessPoolExecutor() as executor:
        for result in executor.map(_analyze_single_file, tasks):
            results.append(result)
            if len(results) % 10 == 0:
                print(f"已处理 {len(results)}/{len(file_paths)}...")

    # 按原始索引排序
    results.sort(key=lambda x: x[0])

    # 分离数据
    valid_results = [r for r in results if r[2] is not None]
    filenames = [r[1] for r in valid_results]
    scores = [r[2] for r in valid_results]

    if not scores:
        print("未能提取到有效数据，无法绘图。")
        return

    print("分析完成，正在生成图表...")

    # --- 设置绘图风格 ---
    plt.style.use('seaborn-v0_8-whitegrid')  # 使用清晰的网格背景

    # 设置中文字体（尝试自动寻找常见中文字体，防止乱码）
    # 如果在Linux/Docker环境可能需要手动指定 .ttf 路径
    font_options = ['SimHei', 'Microsoft YaHei', 'PingFang SC', 'Heiti TC', 'sans-serif']
    plt.rcParams['font.sans-serif'] = font_options
    plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题

    # 创建画布，宽度根据文件数量动态调整，保证不拥挤
    fig_width = max(12, len(filenames) * 0.25)
    fig, ax = plt.subplots(figsize=(fig_width, 7))

    # --- 绘制数据 ---
    x_indices = range(len(filenames))

    # 绘制主折线
    # 颜色使用渐变色或醒目的颜色（如深蓝色）
    ax.plot(x_indices, scores, color='#2878B5', linewidth=2, marker='o', markersize=4,
            label='音高不稳定性 (Cents/Frame)')

    # 添加趋势线（低通滤波或多项式拟合）以观察整体走向
    if len(scores) > 5:
        z = np.polyfit(x_indices, scores, 3)  # 3次多项式拟合
        p = np.poly1d(z)
        ax.plot(x_indices, p(x_indices), "r--", alpha=0.6, linewidth=1.5, label='训练趋势拟合')

    # --- 坐标轴处理 ---
    # X轴
    ax.set_xlim(-0.5, len(filenames) - 0.5)
    # 如果文件太多，X轴标签稀疏显示，或者旋转
    if len(filenames) > 30:
        step = len(filenames) // 30 + 1
        ax.set_xticks(x_indices[::step])
        ax.set_xticklabels(filenames[::step], rotation=45, ha='right', fontsize=9)
    else:
        ax.set_xticks(x_indices)
        ax.set_xticklabels(filenames, rotation=45, ha='right', fontsize=9)

    ax.set_xlabel("模型训练进程 (按轮数排序)", fontsize=12, fontweight='bold')

    # Y轴 - 重点处理数轴标度，避免差异过小看不清
    y_min, y_max = min(scores), max(scores)
    y_range = y_max - y_min
    # 上下各留 10% 的余量，而不是从0开始，这样能放大差异
    ax.set_ylim(y_min - y_range * 0.1, y_max + y_range * 0.1)
    ax.set_ylabel("音高平均抖动量 (Mean Delta F0 in Cents)\n↓ 数值越低越平滑/自然", fontsize=12, fontweight='bold')

    # --- 添加辅助信息与文字描述 ---

    # 标题
    ax.set_title("AI翻唱模型训练过程中的音高/基频稳定性演变", fontsize=16, pad=20)

    # 标出最优值点
    min_score_idx = np.argmin(scores)
    ax.annotate(f'最稳定点\n{filenames[min_score_idx]}',
                xy=(min_score_idx, scores[min_score_idx]),
                xytext=(min_score_idx, scores[min_score_idx] - y_range * 0.2),
                arrowprops=dict(facecolor='green', shrink=0.05),
                ha='center', color='green', fontweight='bold')

    # 文字说明框 (透明背景，不遮挡)
    text_desc = (
        "指标说明：\n"
        "该图表展示了 F0 (基频) 曲线的平均相邻帧差异（以音分为单位）。\n"
        "• 数值高 (Peaks)：表示音高曲线锯齿感强、有断层或非自然抖动，通常对应模型早期或训练崩坏。\n"
        "• 数值低 (Valleys)：表示音高过渡平滑、连续，接近自然歌声。\n"
        "• 趋势：随着训练轮数增加，理想情况下曲线应呈下降趋势或趋于平稳。"
    )

    # 将文字放在图表上方或内部角落，背景透明
    ax.text(0.02, 0.95, text_desc, transform=ax.transAxes, fontsize=10,
            verticalalignment='top', bbox=dict(facecolor='none', edgecolor='gray', alpha=0.5, boxstyle='round,pad=0.5'))

    # 图例
    ax.legend(loc='upper right', framealpha=0.5)

    # 布局紧凑化
    plt.tight_layout()

    # 保存
    plt.savefig(output_image_path, dpi=150)
    print(f"图表已保存至: {output_image_path}")

    # 可选：显示
    # plt.show()


# --- 调用示例 ---
if __name__ == "__main__":
    # 模拟输入数据：请替换为您真实的 wav 文件路径列表
    # 假设这里有50个文件，名称代表epoch
    # demo_files = [f"path/to/model_epoch_{i}.wav" for i in range(100, 5100, 100)]

    # 为了演示代码可运行，这里注释掉调用。
    # 您只需要传入您的 file_paths 数组即可：
    # evaluate_pitch_stability(your_sorted_wav_paths)
    pass